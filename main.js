...
// Función auxiliar para enmarcar el texto con estilo de Zenitsu
const createDecoratedBox = (text) => {
    const top = '╔═ೋ❀❀═══╗';
    const bottom = '╚═ೋ❀❀═══╝';
    const lines = text.split('\n');
    
    // Encuentra la línea más larga para alinear el marco
...
const sendImage = async (conn, to, image, caption = '') => {
    // Si hay un caption, también lo decoramos
...
const sendMessage = async (conn, to, message, type = 'text') => {
    if (type === 'text') {
        ...
        await sendText(conn, to, '¡Ay no! ¡Me congelé del miedo y no sé qué tipo de mensaje es este!');
    }
};
...
async function logEvent(conn, m, type, user = 'Un pobre chico asustado', groupName = '', groupLink = '') {
    console.log(
        ...
        '\n' + chalk.blue('│⏰ Hora del miedo: ') + chalk.green(new Date().toLocaleString('es-ES', { timeZone: 'America/Argentina/Buenos_Aires' })) +
        '\n' + chalk.yellow('️│🏷️ Modo (¡qué nervios!): ') + chalk.magenta([`${conn.public ? 'Público' : 'Privado'}`]) +
        '\n' + chalk.cyan('│📑 Tipo de... ¡Algo está pasando!: ') + chalk.white(type) +
        (m.isGroup ?
            `\n${chalk.bgGreen('│🌸 Grupo (¡espero que no haya demonios!):')} ${chalk.greenBright(groupName)} ➜ ${chalk.green(m.chat)}` +
    ...
    for (const participant of participants) {
        if (action === 'add') {
            ...
            if (welcomeStatus === 'on') {
                ...
                const welcomeMessage = `¡Kyaa! ¡Bienvenido, @${participant.split('@')[0]} a ${groupName}! ¡Espero que este grupo no esté lleno de demonios! ¡Por favor, cuídame!`;
                await sendText(conn, id, welcomeMessage, { mentions: [participant] });
                incrementGrups();
            }
        }
    }
